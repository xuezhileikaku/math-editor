...
<head>
	...
	<link rel="stylesheet" href="./dist/css/eqn-style.css">
	<script src="./dist/js/eqneditor.api.js"></script>
	<style type="text/css">
		...
		.CCToolbar .cc_dropdown img:hover, .cc_dropdown-content img:hover {
		...
		.CCToolbar .cc_dropdown-content.cc_show {
			display: grid;
		}
		...
		#CC_floatingDiv ul .active, #CC_floatingDiv ul li:active, #CC_floatingDiv ul li:focus, #CC_floatingDiv ul li:hover {
			background-color: #003794;
			color: white;
		}
	</style>
	<link rel="stylesheet" href="./dist/css/eqneditor.css">
	<script>
		$(document).ready(function () {
			toolbar = new EqEditor.Toolbar('toolbar');

			textarea = new EqEditor.TextArea('latexInput');
			textarea.addToolbar(toolbar, true);

			exportOutput = new EqEditor.Output('eqcoderaw');
			mainOutput = new EqEditor.Output('output', 'png');
			textarea.addOutput(mainOutput);
			textarea.addOutput(exportOutput);
			textarea.addHistoryMenu(new EqEditor.History('history'));
		});

		function toggleOptions() {
			$('.eOptions').toggle();
			if ($('.eOptions').is(':visible'))
				$('#exportButton').html('Close');
			else
				$('#exportButton').html('Export &raquo;');
		}

		function updateExport(format) {
			var data = exportOutput.exportAs(format);
			$('#exportmessage').text(data.msg);
			$('#eqcoderaw').text(data.str);
		}

		function updateInline(status) {
			if (status === "true") {
				exportOutput.setInline(true);
				mainOutput.setInline(true);
			} else {
				exportOutput.setInline(false);
				mainOutput.setInline(false);
			}
		}
	...
	<link href="https://fonts.googleapis.com/css?family=Noto+Serif:400,400i,700,700i&amp;subset=cyrillic,greek"
		  rel="stylesheet" media="none" onload="if(media!='all')media='all'">
	<link rel="stylesheet" href="./dist/css/style1.min.css?31585757fe0de26dabc5e4d5eaadd9a3">
	<script src="./dist/js/jquery-3.4.0.min.js"></script>
...
<div class="box-header with-border">
	<h3 class="box-title">数学公式编辑器</h3>
	<div class="pull-right box-tools">
		<button>获取代码</button>
	</div>
</div>
<div>
	<h2>EqnEditor (v5 beta)for online mathematics</h2>
	<div id="equation-editor">
		<div id="history">
			<button disabled=""><img alt="Trash Disabled" src="./dist/imgs/ionicons_trash_off.png"
									 style="height: 18px; width: 18px;"></button>
			<button disabled=""><img alt="Undo Disabled" src="./dist/imgs/ionicons_undo_off.png"
									 style="height: 18px; width: 18px;"></button>
			<button disabled=""><img alt="Redo Disabled" src="./dist/imgs/ionicons_redo_off.png"
									 style="height: 18px; width: 18px;"></button>
		</div>
		<div id="toolbar" class="CCToolbar">
		</div>
		<div id="latexInput" autocorrect="off" class="CCTextArea" contenteditable="true" spellcheck="false"
			 tabindex="0"></div>
		<div class="hint">Define equation with <a href="https://en.wikibooks.org/wiki/LaTeX/Mathematics">LaTeX
				markup</a>.<em>&lt;Tab&gt;</em> or <em>&lt;Ctrl+ arrows keys&gt;</em> to jump between brackets and
			matrix elements.
		</div>
		<div id="export_options">
			<div>
				<button id="exportButton" onclick="toggleOptions();">Export »</button>
				<button id="downloadBtn" onclick="exportOutput.download();">Download</button>
			</div>
			<div class="eOptions">
				<select id="format" onchange="exportOutput.setFormat(this.value);" title="Select the output format">
					<option value="svg">svg</option>
					<option value="gif">gif</option>
					<option value="png">png</option>
					<option value="pdf">pdf</option>
				</select>
				<select id="type" onchange="updateExport(this.value);" title="Export Format">
					<!-- <option value="htmledit">HTML (Edit)</option> -->
					<option value="html">HTML</option>
					<option value="url">URL</option>
					<option value="urlencoded">URL Encoded</option>
					<option value="latex">LaTeX</option>
					<option value="pre">PRE</option>
					<option value="tw">TiddlyWiki</option>
					<option value="phpBB">phpBB</option>
					<option value="wp">WordPress</option>
				</select>
				<select id="dpi" onchange="exportOutput.setDpi(this.value); mainOutput.setDpi(this.value);"
						title="Select the output resolution">
				...
				<select id="bg" onchange="exportOutput.setBg(this.value); mainOutput.setBg(this.value);"
						title="Background color">
					<option value="">Transparent</option>
					<option value="bg_white">White</option>
					<option value="bg_black">Black</option>
					<option value="bg_red">Red</option>
					<option value="bg_green">Green</option>
					<option value="bg_blue">Blue</option>
				</select>
				<select id="compressed" onchange="updateInline(this.value);" title="Compression">
					<option value="true">Inline</option>
					<option value="false" selected="">Block</option>
				</select>

				<div id="exportmessage">HTML code to embed this equation into a web page is:</div>
				<div contenteditable="true" id="eqcoderaw"></div>
			</div>
		</div>
...
<div class="container full-height" id="container-block-source">
	<div class="half-width full-height source-wrap" id="source-block">
		<div class="ldt source full-height">
			<pre class="ldt-pre"></pre>
			<label class="ldt-label">
				<textarea id="editor-source" class="ldt-textarea" style="height: 3103px;"></textarea>
			</label>
		</div>
	</div>
	<div class="slider full-height" id="slider" style="position: relative;"></div>
...
<script>
	...
	(function () {
		...
		document.getElementById('editor-source').value = data || "$$|\\vec{A}|=\\sqrt{A_x^2 + A_y^2 + A_z^2}.$$(1)";
	}());
...
<script src="./dist/js/vendors1.min.js"></script>
<script src="./dist/js/scripts1.min.js"></script>
</body>
</html>
